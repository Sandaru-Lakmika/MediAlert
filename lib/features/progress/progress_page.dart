...
class _ProgressPageState extends State<ProgressPage>
  ...
  List<MedicineActivity> _activities = [];
  ...
  void initState() {
    ...
    _progressNotifier.addListener(_onProgressUpdate);
  }

  @override
  void dispose() {
    _progressNotifier.removeListener(_onProgressUpdate);
    super.dispose();
  }
  ...
  Future<double> _calculateAdherence() async {
    if (_medicines.isEmpty) return 0;
    final takenCount = await ActivityService.getTodayTakenCount();
    final totalDoses = _getTodayDoses();
    if (totalDoses == 0) return 0;
    return (takenCount / totalDoses * 100).clamp(0, 100);
  }
  ...
  Future<int> _getCompletedDoses() async {
    return await ActivityService.getTodayTakenCount();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<int>(
      future: _getCompletedDoses(),
      builder: (context, completedSnapshot) {
        final completedDoses = completedSnapshot.data ?? 0;
        final totalDoses = _getTodayDoses();
  ...
  List<Widget> _buildRecentActivity(ColorScheme cs, TextTheme textTheme) {
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Start taking your medicines to track progress',
                style: textTheme.bodySmall?.copyWith(
    ...
    final recentActivities = _activities.take(10).toList();

    return recentActivities.map((activity) {
      final isTaken = activity.action == 'taken';
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    isTaken
                        ? '${activity.medicineName} taken'
                        : 'Missed: ${activity.medicineName}',
                    style: textTheme.bodyLarge?.copyWith(
                      fontWeight: FontWeight.w600,
                      color: cs.onSurface,
                    ),
